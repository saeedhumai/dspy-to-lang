...
class SocketManager:
    ...
    async def handle_connect(self, sid, environ):
        ...
        try:
            ...
            logger.info(f"Query string: {environ['QUERY_STRING']}")
            query = environ.get('QUERY_STRING', b'')
            logger.info(f"Query string: {query}")
            # Only split non-empty parameters and handle URL-encoded values
            ...
            if user_id and user_id != 'undefined':  # Add check for 'undefined'
                ...
                logger.warning(f"Connection attempt without valid user_id: {sid}")
